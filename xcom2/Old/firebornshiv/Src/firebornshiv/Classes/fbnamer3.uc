class fbnamer3 extends XGCharacterGenerator
	dependson(X2StrategyGameRulesetDataStructures);

var localized string fbSparkFirstName;
var localized string fbSparkLastNamePrefix;
...
function GenerateName( int iGender, name CountryName, out string strFirst, out string strLast, optional int iRace = -1 )
{
	...
	strFirst = fbSparkFirstName;
	strLast = fbSparkLastNamePrefix;
	SparkIndex = 1;
	bUseRandomNum = true;
	...
	if(History != none)
	{
		...
		if(XComHQ != none)
		{
			...
			for(idx = 0; idx < XComHQ.Crew.Length; idx++)
			{
				...
				if(UnitState != none && UnitState.GetMyTemplateName() == 'shiv_m2')
				{
					SparkIndex++;
				}
			}
